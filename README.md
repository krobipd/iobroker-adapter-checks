...
Each check reads only the adapter it is pointed at and never writes. `object-rewrite` parses the
sources with the adapter's own `typescript` (an optional peer dependency — every TypeScript adapter
has it); when no compiler can be loaded it reports that as a finding instead of staying silent.
